		...
		try {
			if(item != null){
				...
				for(ShopItem p : listp){
					...
					pro.setUrl(p.getUrl());
					pro.setCreatedate(getCalendar().getTime());
					pro.setYear(getCalendar().get(Calendar.YEAR)+"");
					pro.setMonth((getCalendar().get(Calendar.MONTH)+1)+"");
					pro.setDay(getCalendar().get(Calendar.DAY_OF_MONTH)+"");
					pro.setHour(getCalendar().get(Calendar.HOUR)+"");
					pro.setMinutes(getCalendar().get(Calendar.MINUTE)+"");
					pro.setSecond(getCalendar().get(Calendar.SECOND)+"");
